...
@Component({
  ...
  templateUrl: './profesor-editar.html',
  styleUrls: ['./profesor-editar.css']
})
export class ProfesorEditarComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.svc.obtener(this.id).subscribe({
      ...
      error: _ => { this.loading = false; alert('No se pudo cargar'); this.router.navigateByUrl('/profesor'); }
    });
  }
